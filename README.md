...
## 🛰️ How to access in mobile 

This setup is specifically intended for mobile access because service workers only work on **HTTPS** or **localhost**. Accessing your local server via a physical IP address does not meet this requirement, so Ngrok provides a secure HTTPS tunnel to your local server.

### Step 1: Start Ngrok Tunnel

1. Open a terminal.
2. Authenticate Ngrok (first-time setup) with your token from the Ngrok dashboard:
   ```bash
   ngrok authtoken YOUR_NGROK_TOKEN
   ```
3. Start an HTTP tunnel pointing to your Apache port:
   ```bash
   ngrok http 80   # Replace 80 with your Apache port if different
   ```

4. Ngrok will generate a forwarding URL like:
   ```
...
3. Enter the HTTPS URL generated by Ngrok (e.g., `https://abc123.ngrok.io`).
4. Your XAMPP site should load securely over HTTPS.
